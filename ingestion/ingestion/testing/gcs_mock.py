...
# - Client.from_service_account_json class method
# - Blob.crc32 property (only populated on Blob.upload_from_string)
...
import google.cloud.storage.retry  # type: ignore[import-untyped]
...
class Client(mock.MagicMock):
    ...
    def from_service_account_json(
        cls, json_credentials_path: str, *, _env: Environment
    ) -> Client:
        return cls(_env=_env)
...
class Bucket(mock.MagicMock):
    ...
    def get_blob(
        ...
        timeout=google.cloud.storage.constants._DEFAULT_TIMEOUT,
        ...
        **kwargs,
        ...
        fp = blob._get_local_path_opt(readable=True)
        if fp is None:
            return None

        blob._update_hash(fp)
        return blob
...
class Blob(mock.MagicMock):
    ...
    def _get_local_path_opt(
        self, readable: bool = False, writable: bool = False
    ) -> pathlib.Path | None:
        """Returns the local path of this blob or None if not exists."""
    ...
    def _update_hash(self, local_path: pathlib.Path) -> None:
        with open(local_path, "rb") as fp:
            fp = Crc32cCalculator(fp)
            fp.read()
        self.crc32c = fp.hexdigest()
...
def patch(
    ...
    with contextlib.ExitStack() as stack:
        for name in client_cls_names:
            ...
            mocked.from_service_account_json = (
                lambda *args, **kwargs: Client.from_service_account_json(
                    *args, _env=env, **kwargs
                )
            )
