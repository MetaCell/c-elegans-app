...
import {
	type SynchronizerContext,
	SynchronizerOrchestrator,
} from "./synchronizer";

function triggerUpdate<T extends Workspace>(
	_prototype: any,
	_key: string,
...
export class Workspace {
	...
	constructor(
		...
		this.visibilities =
			...
			Object.fromEntries(
				[...(activeNeurons || [])].map((n) => [
					n,
					getDefaultViewerData(Visibility.Visible),
				]),
			);
	...
	hideNeuron(neuronId: string) {
		...
		this.visibilities[neuronId][ViewerType.Graph].visibility =
			Visibility.Hidden;
		this.visibilities[neuronId][ViewerType.ThreeD].visibility =
			Visibility.Hidden;
	...
	@triggerUpdate
	updateViewerSynchronizationStatus(
	...
	async _getAvailableNeurons() {
		try {
			...
			this.availableNeurons = Object.fromEntries(
				[...uniqueNeurons].map((n) => [n.name, n]),
			);
		} catch (error) {
	...
	getNeuronCellsByClass(neuronClassId: string): string[] {
		return Object.values(this.availableNeurons)
			.filter(
				(neuron) =>
					neuron.nclass === neuronClassId && neuron.nclass !== neuron.name,
			)
	...
	getVisibleNeuronsInThreeD(): string[] {
		return Array.from(this.activeNeurons).filter(
			(neuronId) =>
				this.visibilities[neuronId]?.[ViewerType.ThreeD]?.visibility ===
				Visibility.Visible,
		);
	...
	getVisibleNeuronsInEM(): string[] {
		return Array.from(this.activeNeurons).filter(
			(neuronId) =>
				this.visibilities[neuronId]?.[ViewerType.EM]?.visibility ===
				Visibility.Visible,
		);
	}
			...
			for (const viewerType of viewers) {
				if (viewerType in draft.visibilities[neuronId]) {
					...
					if (
						viewerData &&
						"color" in viewerData &&
						typeof viewerData.color === "string"
					) {
