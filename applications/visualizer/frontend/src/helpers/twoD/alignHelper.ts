...
export const distributeNeurons = (alignment: Alignment, selectedNeurons: string[], cy: Core) => {
    ...
    const minPos = alignment === Alignment.Horizontal
        ? cyNodes[0].position('x')
        : cyNodes[0].position('y');
    const maxPos = alignment === Alignment.Horizontal
        ? cyNodes[cyNodes.length - 1].position('x')
        : cyNodes[cyNodes.length - 1].position('y');
